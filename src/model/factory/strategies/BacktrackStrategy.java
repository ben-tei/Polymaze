...
public class BacktrackStrategy extends MazeFactoryStrategy {
	...
	/**
	 * Recursive method to "explore" all unvisited neighbour of the current Cell.
	 * @param x
	...
	public void exploreMaze(int x, int y) {
		...
		while (this.getNonVisitedNeighbour(this.mazeArray, x, y).length() > 0) {
			...
			char direction = currentNeighbour
					.charAt((int) (Math.random() * currentNeighbour.length()));
			if (direction == 'N') {
	...
	/**
	 * @param mazeArray2 Cell[][] the array of the maze
